...
%% PIAACMC Model with GSNum 189 PR
...
params.opticalPrescriptionFileName = 'opticalElements_luvoirA_piaacmc_compactLyot.xlsx'; % read excel file with elements
params.padfactor = 2;

% definition of primary
params.primary.D = 2*14.2e-3; % everything is in collimated space with respect to the PIAA mirrors
...
params.DM(1).diameter = 2*14.2e-3; % resized to PIAA diameter
...
params.illumination.pupilSurf = 'opticalElements\luvoirApiaacmc\zeros_n2048.fits';
params.illumination.nuTekSag1 = 'opticalElements\luvoirApiaacmc\zeros_n2048.fits';
params.illumination.nuTekSag2 = 'phaseRetrieval\piaa_127phwraprescale_procSag-direct_n2048.fits';
...
% use measured influence function
labInfFunc = fitsread('DMresources\labInfFuncResampled2D.fits');
system.optics.elem(system.params.DM(1).elemID).ifArr.if = labInfFunc;
...
%system.optics.elem(1).sag = 'phaseRetrieval\piaa_189surf2_n2048.fits';
...
simOptions.useab = 1;
system.outputAb = propagate_optics(system, lambda0, simOptions);
simOptions.useab = 0;
...
figure(); imagesc(system.sci.xlD, system.sci.ylD, log10(system.outputAb.psfE.*conj(system.outputAb.psfE)./system.I00)); axis image;
set(gcf,'color','white')
title('LUVOIR PSF Abb', 'FontSize', 16)
xlabel('Sky Angular Separation, $\lambda_0 / D$', 'Interpreter', 'latex', 'FontSize', 16)
ylabel('Sky Angular Separation, $\lambda_0 / D$', 'Interpreter', 'latex', 'FontSize', 16)
set(gcf,'color', 'white')
colorbar
